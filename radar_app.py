...
lst_ok1 = ['EDDF', 'EGLL', 'EHAM', 'EKCH', 'EPWA', 'ESSA', 'KJFK', 'KLAX', 'KORD', 'KSEA', 'KSFO', 'LEMD', 'LFPG', 'LGAV', 'LIMC', 'LIRF', 'LKPR', 'LOWW', 'LSZH', 'NZAA', 'OMDB', 'PAFA', 'PANC', 'PASY', 'UUEE', 'UMMS', 'YMML', 'YSSY']
lst_ok2 = []
lst_ok3 = []
lst_ok4 = []
dct_ok1 = {}
...
for a in range(0, len(lst_ok1)):
    url1 = f'http://www.gcmap.com/airport/{lst_ok1[a]}'
    ...
    dct_ok1[f'{lst_ok1[a]}'] = {'City': f'{city}', 'Region': f'{region}', 'Country': f'{country}', 'Lat': f'{lat}', 'Lon': f'{lon}', 'Elevation': f'{elev}'}
...
if region == 'OK1':
    ...
    while i <= len(dct_ok1)-1:
        ...
        if cols[0].button(f'{list(dct_ok1.keys())[i]}'):
            placeholder1.title(f'{(dct_ok1.get(lst_ok1[i])).get("City")}, {(dct_ok1.get(lst_ok1[i])).get("Country")}')
        ...
        if cols[1].button(f'{list(dct_ok1.keys())[i+1]}'):
            placeholder1.title(f'{(dct_ok1.get(lst_ok1[i+1])).get("City")}, {(dct_ok1.get(lst_ok1[i+1])).get("Country")}')
        ...
        if cols[2].button(f'{list(dct_ok1.keys())[i+2]}'):
            placeholder1.title(f'{(dct_ok1.get(lst_ok1[i+2])).get("City")}, {(dct_ok1.get(lst_ok1[i+2])).get("Country")}')
...
# region = st.sidebar.selectbox('',
#                               ('------------------------------------>>', 'OK1', 'OK2', 'OK3', 'OK4'))
# b1, b2, b3 = st.sidebar.beta_columns(3)
# b4, b5, b6 = st.sidebar.beta_columns(3)
# b7, b8, b9 = st.sidebar.beta_columns(3)
# b10, b11, b12 = st.sidebar.beta_columns(3)
# b13, b14, b15 = st.sidebar.beta_columns(3)
# b16, b17, b18 = st.sidebar.beta_columns(3)
# b19, b20, b21 = st.sidebar.beta_columns(3)
# b22, b23, b24 = st.sidebar.beta_columns(3)
# b25, b26, b27 = st.sidebar.beta_columns(3)
# if region == 'OK1':
...
#         placeholder2.markdown(f'<iframe src="https://www.rainviewer.com/map.html?loc={var},22.4121,4&oFa=1&oC=1&oU=1&oCS=1&oF=1&oAP=1&rmt=1&c=5&o=49&lm=0&th=1&sm=0&sn=1" width="100%" frameborder="0" style="border:0;height:88vh;" allowfullscreen></iframe>', unsafe_allow_html=True)
